...
class EulerAncestralDiscreteScheduler {
  constructor(schedulerConsts, latentShape, tvm, device, vm) {
    ...
    this.ScaleModelInputFunc = tvm.detachFromCurrentScope(
      vm.getFunction("euler_ancestral_discrete_scheduler_scale")
    )
    this.stepFunc = tvm.detachFromCurrentScope(
      vm.getFunction("euler_ancestral_discrete_scheduler_step")
    )
  }
  ...
  step(modelOutput, sample, counter, noise) {
    const prevLatents = this.stepFunc(
      sample,
      modelOutput,
      this.sigma[counter],
  ...
  async generate(
    ...
    if (schedulerId == 3) {
      scheduler = new EulerAncestralDiscreteScheduler(
        this.schedulerConsts[3], latentShape, this.tvm, this.device, this.vm);
      unetNumSteps = this.schedulerConsts[3]["num_steps"];
    } else {
      //raise error
    ...
    // use uniform distribution with same variance as normal(0, 1)
    const scale = Math.sqrt(12) / 2 * 14.6146;
    let latents = this.tvm.detachFromCurrentScope(
      this.tvm.uniform(latentShape, -scale, scale, this.tvm.webgpu())
    );
    this.tvm.endScope();
    ...
    for (let counter = 0; counter < unetNumSteps; ++counter) {
      ...
      const newLatents = this.tvm.withNewScope(() => {
        ...
        const array_id = [512., 512., 0., 0., 512., 512.]
        let add_time_ids = this.tvm.empty([1, 6], "float32", this.tvm.webgpu()).copyFrom(array_id);
        const noisePred = this.unetLatentsToNoisePred(
          scaled_latent_model_input, timestep, embeddings, 
            pool_embeddings, add_time_ids, this.unetParams);
        // maintain new latents
        const noiseScale = Math.sqrt(12) / 2;
        const noise = this.tvm.uniform(latentShape, -noiseScale, noiseScale, this.tvm.webgpu());
        return this.tvm.detachFromCurrentScope(
          scheduler.step(noisePred, latents, counter, noise)
        );
      });
      latents = newLatents;
      // use skip one sync, although likely not as useful.
  ...
  async #asyncInitPipeline(schedulerConstUrl, tokenizerName, tokenizerName2) {
    ...
    if (this.model == "SDXL-turbo") {
      ...
      this.pipeline = this.tvm.withNewScope(() => {
        return new SDXLturboPipeline(this.tvm, tokenizer1, tokenizer2, schedulerConst, this.tvm.cacheMetadata);
      });
